...
### 실거래(국토부) ↔ 단지 매칭
소스마다 단지명이 다르다(건축물대장 "잠실아파트" vs 국토부 실거래 "주공아파트 5단지"). 이름 매칭은 불안정.
- **1순위: 지번(jibun) 일치** — 국토부 실거래 API의 `jibun` 필드로 매칭. 이름 무관, 가장 정확.
  - 앞 0 제거 정규화 후 비교. 클릭/단지 지번 = 실거래 지번.
- **2순위: 단지명 근사매칭** — 지번으로 못 잡을 때만 (같은 지번 여러 단지 등).
- **3순위: 사용자 선택** — 자동 실패 시 화면에 동(洞) 실거래 단지명 후보 목록 → 사람이 선택.
- 구현: `lib/complexCollector.ts` `collectTrade(lawd, dong, aptNm, jibun)`. 반환 `matchBy`로 매칭방식 표기(신뢰도).
- 해제거래(cdealType) 제외.

### 법정동코드(bjdong) 확정
- 건축물대장 API로 후보 bjdong 순회 → 응답 platPlc의 동명이 입력 동명과 일치하는 코드 채택. (추측 금지)
