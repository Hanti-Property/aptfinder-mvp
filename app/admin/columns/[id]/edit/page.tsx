...
function RichEditor({
  ...
  initialHtml: string; onChange: (html: string) => void; placeholder: string
...
export default function ColumnEditPage() {
  ...
  const [form, setForm] = useState({ series: '', title: '', author: '', author_field: '' })
  ...
  useEffect(() => {
    async function fetchColumn() {
      const { data, error } = await supabase.from('expert_columns').select('*').eq('id', id).single()
      if (error || !data) { setNotFound(true); setLoading(false); return }
      setForm({ series: data.series || '', title: data.title || '', author: data.author || '홍성욱', author_field: data.author_field || '공인중개사' })
  ...
  async function handleSubmit() {
    ...
    let { error } = await supabase.from('expert_columns').update({ ...base, thumbnail: thumbUrl || null }).eq('id', id)
    if (error && /thumbnail/.test(error.message)) {
      ;({ error } = await supabase.from('expert_columns').update(base).eq('id', id))
    }
    setSaving(false)
    if (error) alert('수정 실패: ' + error.message)
    else { alert('✅ 칼럼이 수정되었습니다!'); router.push('/admin/columns/list') }
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <header className="bg-[#1B3A5C] text-white px-6 py-4 flex items-center justify-between">
        <h1 className="text-lg font-semibold">칼럼 수정</h1>
      ...
      <div className="max-w-5xl mx-auto p-6">
        {preview ? (
          <div className="max-w-2xl mx-auto bg-white border border-gray-200 rounded-lg p-6">
            ...
            <div className="mb-2">
              <span className="text-[11px] text-white px-2 py-0.5 rounded bg-[#C49A3C]">{form.series || '시리즈 미선택'}</span>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-5">
              <div>
                ...
                <RichEditor initialHtml={initContent} onChange={setContentHtml} minHeight={360}
