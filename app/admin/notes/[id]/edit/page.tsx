...
/** 재사용 서식 에디터. initialHtml로 최초 1회 기존값 주입(수정 화면용). */
function RichEditor({
  initialHtml, onChange, placeholder, minHeight = 200, tools, bg = 'bg-white',
}: {
  initialHtml: string
  ...
  useEffect(() => {
    if (ref.current && !seeded.current && initialHtml) {
  ...
  const label: Record<ToolKey, { t: string; cls?: string; title: string }> = {
    bold: { t: 'B', cls: 'font-bold', title: '굵게' },
    italic: { t: 'I', cls: 'italic', title: '기울임' },
    underline: { t: 'U', cls: 'underline', title: '밑줄' },
    h3: { t: 'H', title: '제목' },
    ul: { t: '• 목록', title: '목록' },
    quote: { t: '❝', title: '인용' },
    link: { t: '🔗', title: '링크' },
    clear: { t: '✕', title: '서식 지우기' },
  }
      ...
      <div className="flex flex-wrap gap-1 mt-1.5 p-2 border border-gray-300 border-b-0 rounded-t-lg bg-gray-50">
        {tools.map(k => (
          <button key={k} type="button" title={label[k].title}
            className={`${btnCls} ${label[k].cls || ''}`}
            onMouseDown={e => onTool(k, e)}>
            {label[k].t}
          </button>
        ))}
      </div>
...
export default function NoteEditPage() {
  ...
  const [initBottom, setInitBottom] = useState('')
  ...
  useEffect(() => {
    async function fetchNote() {
      const { data, error } = await supabase.from('research_notes').select('*').eq('id', id).single()
      if (error || !data) { setNotFound(true); setLoading(false); return }
      setForm({ title: data.title || '', category: data.category || '' })
      setContentHtml(data.content || ''); setInitContent(data.content || '')
      setBottomHtml(data.bottom_line || ''); setInitBottom(data.bottom_line || '')
  ...
  async function handleSubmit() {
    if (!form.title || !form.category || !stripHtml(contentHtml)) {
      alert('제목, 카테고리, 본문은 필수입니다.')
      return
    }
    setSaving(true)
    const base = { title: form.title, category: form.category, content: contentHtml, bottom_line: bottomHtml }
    let { error } = await supabase.from('research_notes').update({ ...base, thumbnail: thumbUrl || null }).eq('id', id)
    if (error && /thumbnail/.test(error.message)) {
      ;({ error } = await supabase.from('research_notes').update(base).eq('id', id))
    }
    setSaving(false)
    if (error) alert('수정 실패: ' + error.message)
    else { alert('✅ 노트가 수정되었습니다!'); router.push('/admin/notes/list') }
  }
      ...
      <div className="max-w-5xl mx-auto p-6">
        {preview ? (
          <div className="max-w-2xl mx-auto bg-white border border-gray-200 rounded-lg p-6">
            ...
            <div className="mb-3">
              <span className={`text-[11px] text-white px-2 py-0.5 rounded ${catColor[form.category] || 'bg-gray-500'}`}>
                {form.category || '카테고리 미선택'}
              </span>
            </div>
            <h1 className="text-xl font-bold mb-4 text-gray-900">{form.title || '(제목 없음)'}</h1>
            {thumbUrl && (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-1 space-y-5">
              ...
              <div>
                ...
                <input type="text" className="w-full p-3 border border-gray-300 rounded-lg mt-1.5"
            ...
            <div className="lg:col-span-2 space-y-5">
              <div>
                ...
                <RichEditor initialHtml={initContent} onChange={setContentHtml} minHeight={340}
                  placeholder="핵심 내용을 간결하게 작성하세요."
                  tools={['bold', 'italic', 'underline', 'h3', 'ul', 'quote', 'link', 'clear']} />
              </div>
              <div>
                <label className="text-sm font-medium text-gray-700">📌 Bottom Line</label>
                <RichEditor initialHtml={initBottom} onChange={setBottomHtml} minHeight={130} bg="bg-blue-50"
                  placeholder="이 정보가 매수/전세/투자 판단에 어떤 의미인지 결론을 제시하세요."
